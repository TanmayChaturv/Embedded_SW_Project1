...
status buff_init( circbuf_t *ptr, int16_t length )
{
		...
		{
			ptr->buffer = (int16_t*)malloc(sizeof(int16_t)*length);
			if ( ptr->buffer == NULL )
			{
				return BUFF_INIT_FAIL;
			}
...
*/
status buff_insert( circbuf_t *ptr, int16_t data )
{
